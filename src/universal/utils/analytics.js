...
// Events which are incremented
const incrementedTrackingEvents = [
  ADDED_TO_CART,
  CLICKED_CHECKOUT,
]
...
const gaEvent = (eventName) => {
  if (__PRODUCTION__ && typeof ga !== 'undefined') {
    ga('send', 'event', promo.name, eventName)
  }
}
const gaIdentify = (id) => {
  if (__PRODUCTION__ && typeof ga !== 'undefined') {
...
const mixpanelUnion = (category, eventName) => {
  if (__PRODUCTION__ && typeof mixpanel !== 'undefined') {
    mixpanel.people.union(category, eventName)
  }
}
...
const mixpanelPage = (pageName, data) => {
  if (__PRODUCTION__ && typeof mixpanel !== 'undefined') {
    ...
    mixpanelUnion('Pages Visited', `${promo.name}`)
  }
}
const mixpanelTrack = (eventName, data) => {
  if (__PRODUCTION__ && typeof mixpanel !== 'undefined') {

    // Perform regular track
    mixpanel.track(eventName, Object.assign({}, data, {
      promo: promo.name,
    }))

    // Increment people property if appropriate
    if (incrementedTrackingEvents.includes(eventName)) {
      mixpanelIncrement(eventName)
    }
...
analyticsEvents.on(LANDER_PAGE, (data) => {
  ...
  // Google Analytics
  let pageTitle = `${promo.name} - ${LANDER_PAGE}`
  let pageData = {
...
analyticsEvents.on(SALE_PAGE, (data) => {
  ...
  // Google Analytics
  let pageTitle = `${promo.name} - ${SALE_PAGE}`
  let pageData = {
    page: document.location.pathname,
    location: `${window.location.protocol}//${window.location.host}${window.location.pathname}`,
    title: pageTitle,
...
analyticsEvents.on(OPTIONS_PAGE, (data) => {
  ...
  // Google Analytics
  let pageTitle = `${promo.name} - ${OPTIONS_PAGE}`
  gaPage(pageTitle, {
    page: document.location.pathname,
    location: `${window.location.protocol}//${window.location.host}${window.location.pathname}`,
    title: pageTitle,
