...
const { leadId, cookieLeadId, emailIdKey } = leadStorageOptions
...
export function createLead(dispatch, variables) {
  ...
  return new Promise(async function (resolve, reject) {
    const query = `
    mutation($email: Email!, $age: String!, $source: String, $keyword: String, $listId: String!){
      payload: createLead(email: $email, source: $source, keyword: $keyword, listId: $listId)
      ${lead}
    }`
    ...
    if (error) {
      ...
      if (__CLIENT__) {
        ...
        if (localStorage) {
          localStorage.setItem(leadId, payload.id)
          localStorage.setItem(emailIdKey, payload.email)
        }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (error) {
      ...
      if (__CLIENT__) {
        ...
        const analyticsData = Object.assign({}, payload, {
...
export const syncLeadByEmail = (dispatch, variables) => {
  ...
  // Perform mixpanel identify regardless of db lookup success
  identify(variables.email, {})
